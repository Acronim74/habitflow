...
function renderToday() {
  ...
  if (activeGoal) {
    ...
    if (goalSched.length > 0) {
      ...
      hdr.innerHTML = `<span class="gth-icon">${goal.icon}</span><span class="gth-name">${esc(goal.name)}</span><button class="gth-link" type="button" onclick="navigate('goals')">Этап ${activeGoal.stage} · подробнее →</button>`;
      goodList.appendChild(hdr);
    }

    const _appendSection = (list) => {
...
function _buildGoalsList() {
  return `<div class="goals-wrap">
    <h2 class="goals-title">Выбери цель</h2>
    <p class="goals-sub">Готовый 63-дневный трек — привычки, которые приведут к результату шаг за шагом.</p>
        ...
        <div class="goal-card">
          <div class="goal-card-top">
            ...
            <div class="goal-card-info">
              ...
              <div class="goal-card-meta">63 дня · 3 этапа · 9 привычек</div>
            </div>
          </div>
          ...
          <div class="goal-card-habits">
            ${g.stages[0].habits.map(h => {
              const freq = getHabitFreq(g.id, h.key, 'ru');
          ...
          <button type="button" class="btn btn-primary goal-card-btn" onclick="openGoalSurvey('${g.id}')">Начать →</button>
        </div>`).join('')}
    </div>
  </div>`;
}
  ...
  const stagesHtml = goal.stages.map(s => {
    if (s.n > activeGoal.stage) {
      return `<div class="gsi gsi-locked">
        <div class="gsi-head">
          <span class="gsi-num">Этап ${s.n}</span>
          <span class="gsi-name">${esc(s.name)}</span>
          <span class="gsi-days">Дни ${s.days}</span>
    ...
    const habitsHtml = stageHabits.map(h => {
      ...
      const why  = h.goalHabitKey ? getHabitWhy(h.goalId, h.goalHabitKey, 'ru') : '';
      const freq = h.goalHabitKey ? getHabitFreq(h.goalId, h.goalHabitKey, 'ru') : '';
  ...
  return `<div class="goals-wrap">
    <div class="goal-active-hdr">
      ...
      <div class="gah-info">
        ...
        <div class="gah-day">День ${dayN} из 63</div>
      </div>
    </div>
    ...
    <button type="button" class="btn btn-ghost goal-abandon-btn" onclick="confirmAbandonGoal()">Остановить цель</button>
  </div>`;
}
